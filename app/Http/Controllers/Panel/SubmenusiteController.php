...
class SubmenusiteController extends Controller
{
    ...
    {
        $thispage       = [
            'title'   => 'مدیریت زیرمنو سایت',
            'list'    => 'لیست زیرمنو سایت',
            'add'     => 'افزودن زیرمنو سایت',
            'create'  => 'ایجاد زیرمنو سایت',
            'enter'   => 'ورود زیرمنو سایت',
            'edit'    => 'ویرایش زیرمنو سایت',
            'delete'  => 'حذف زیرمنو سایت',
        ];
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                ...
                ->editColumn('action', function ($data) {
                    ...
                    $actionBtn = '';
                    if (auth()->user()->can('can-access', ['submenusite', 'edit'])) {
                        $actionBtn .= '<button type="button" class="'.$base.' btn btn-sm btn-outline-primary edit-btn" data-id="'.$data->id.'" data-url="'.route('submenusite.edit', $data->id).'"><i class="mdi mdi-pencil-outline"></i></button>';
                    }
                    if (auth()->user()->can('can-access', ['submenusite', 'delete'])) {
        ...
        return view('panel.submenusite')->with(compact(['thispage']));
    }
...
//        return redirect(route('menudashboards.index'));
    ...
    {
        $menupanels    = Menu::all();
        $submenusite  = Submenu::whereId($id)->first();

        return view('panel.partials.edit-form-submenusite', compact('menupanels', 'submenusite'));
    }
